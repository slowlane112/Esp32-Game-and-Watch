...
esp_lcd_panel_handle_t setup_lcd_1_spi() {		// ili9341 lcd screen 1
...
esp_lcd_panel_handle_t setup_lcd_2_spi() {		// ili9341 lcd screen 2
...
/*
esp_lcd_panel_handle_t setup_lcd_1_spi() {		// st7789 lcd screen 1
...
esp_lcd_panel_handle_t setup_lcd_2_spi() {		// st7789 lcd screen 2
...
void app_main(void)
{
	...
	// Load ROM
	extern const uint8_t rom_gw_start[] asm("_binary_gnwoilpanic_gw_start");
	extern const uint8_t rom_gw_end[] asm("_binary_gnwoilpanic_gw_end");
	...
	while (true) {
		...
		if (display_update_count == 8) {
			...
			esp_lcd_panel_draw_bitmap(spi_lcd_1_handle, 0, RENDER_PADDING, GW_SCREEN_WIDTH, RENDER_HEIGHT + RENDER_PADDING, framebuffer);
			esp_lcd_panel_draw_bitmap(spi_lcd_2_handle, 0, RENDER_PADDING, GW_SCREEN_WIDTH, RENDER_HEIGHT + RENDER_PADDING, framebuffer + (GW_SCREEN_WIDTH * RENDER_HEIGHT));
