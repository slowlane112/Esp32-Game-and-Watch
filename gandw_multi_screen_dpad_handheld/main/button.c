...
uint8_t button_get_menu_buttons()
{
	...
	if (button_start_delay == 0) {
		...
		else if (gpio_get_level(BUTTON_LEFT) == 0 && gpio_get_level(BUTTON_JUMP) == 0) {
...
unsigned int button_get_dpad_buttons()
{
    ...
    if (button_start_delay == 0) {
		...
		if (gpio_get_level(BUTTON_LEFT) == 0 && gpio_get_level(BUTTON_JUMP) == 0) { 
