...
#define GW_ROM_SIZE_MAX 130000U
#define GW_ROM2_SIZE_MAX 30000U
#define GW_ROM3_SIZE_MAX 10000U
unsigned char *GW_ROM;
unsigned char *GW_ROM2;
unsigned char *GW_ROM3;
...
int segments_pixel_start = 0;
int segments_offset_start = 0;

bool gw_romloader_rom2ram()
{
	
	/*
	 * The Cardputer does not have PSRAM and only has just enough memory to display Game and Watch multi‑screen games on two 240x135 displays.
	 * After allocating memory for the framebuffer, the Cardputer doesn't have enough contiguous memory to fit the game ROM, so I've split it into three parts.
	 * Due to memory constraints, the Cardputer cannot decompress a compressed game file, so you must use uncompressed .gw files. These can be found in the LCD‑Game‑Shrinker build directory.
	 */

   GW_ROM = malloc(GW_ROM_SIZE_MAX);
   GW_ROM2 = malloc(GW_ROM2_SIZE_MAX);
   GW_ROM3 = malloc(GW_ROM3_SIZE_MAX);

   /* src pointer to the ROM data in the external flash (raw or LZ4) */
   const unsigned char *src = (unsigned char *)ROM_DATA;

   // get head so we can spilt game rom
   memcpy(&gw_head, src, sizeof(gw_head));

   /* dest pointer to the ROM data in the internal RAM (raw) */
   unsigned char *dest = (unsigned char *)GW_ROM;
   unsigned char *dest2 = (unsigned char *)GW_ROM2;
   unsigned char *dest3 = (unsigned char *)GW_ROM3;
   ...
   if (memcmp(src, ROM_CPU_SM510, 3) == 0)
   {
      ...
      segments_pixel_start = gw_head.segments_pixel;
      segments_offset_start = gw_head.segments_offset;
      
      // header and background
      memcpy(dest, src, segments_pixel_start);
      // segment pixel data
      memcpy(dest2, src + segments_pixel_start, segments_offset_start - segments_pixel_start);
      // rest	
      memcpy(dest3, src + segments_offset_start, ROM_DATA_LENGTH - segments_offset_start);
   ...
   // modified for spilt game rom
   gw_segments = (unsigned char *)&GW_ROM2[gw_head.segments_pixel - segments_pixel_start];
   
   gw_segments_offset = (unsigned int *)&GW_ROM3[gw_head.segments_offset - segments_offset_start];
   gw_segments_x = (unsigned short *)&GW_ROM3[gw_head.segments_x - segments_offset_start];
   gw_segments_y = (unsigned short *)&GW_ROM3[gw_head.segments_y - segments_offset_start];
   gw_segments_width = (unsigned short *)&GW_ROM3[gw_head.segments_width - segments_offset_start];
   gw_segments_height = (unsigned short *)&GW_ROM3[gw_head.segments_height - segments_offset_start];


   gw_program = (unsigned char *)&GW_ROM3[gw_head.program - segments_offset_start];

   if (gw_head.melody_size != 0)
      gw_melody = (unsigned char *)&GW_ROM3[gw_head.melody - segments_offset_start];

   gw_keyboard = (unsigned int *)&GW_ROM3[gw_head.keyboard - segments_offset_start];
