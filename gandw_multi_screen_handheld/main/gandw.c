...
uint8_t read_selected_game() {
    uint8_t selected_game = 1; // gnwoilpanic
    nvs_handle_t my_handle;
    ESP_ERROR_CHECK(nvs_open("gandw", NVS_READWRITE, &my_handle));
    nvs_get_u8(my_handle, "selected_game", &selected_game);
    nvs_close(my_handle);

    if (selected_game < 1 || selected_game > 3) {
        selected_game = 1;
    }
...
void load_game() {
    ...
    extern const uint8_t gnwoilpanic_gw_start[] asm("_binary_gnwoilpanic_gw_start");
    extern const uint8_t gnwoilpanic_gw_end[] asm("_binary_gnwoilpanic_gw_end");
    extern const uint8_t gnwsafebuster_gw_start[] asm("_binary_gnwsafebuster_gw_start");
    extern const uint8_t gnwsafebuster_gw_end[] asm("_binary_gnwsafebuster_gw_end");
    extern const uint8_t gnwpinball_gw_start[] asm("_binary_gnwpinball_gw_start");
    extern const uint8_t gnwpinball_gw_end[] asm("_binary_gnwpinball_gw_end");
